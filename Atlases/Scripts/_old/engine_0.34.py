...
while running:
    ...
    # Inside the main loop, update puck positions and check for inactivity
    for puck in pucks:
        if puck.active:
            ...
            # Check for edge-of-screen collisions and provide velocity boost
            if (puck.x - puck.radius < 0) and puck.vx < 0:
                puck.vx = -puck.vx
            elif (puck.x + puck.radius > WIDTH) and puck.vx > 0:
                puck.vx = -puck.vx

            if (puck.y - puck.radius < 0) and puck.vy < 0:
                puck.vy = -puck.vy
            elif (puck.y + puck.radius > HEIGHT) and puck.vy > 0:
                puck.vy = -puck.vy
