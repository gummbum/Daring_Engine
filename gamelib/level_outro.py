...
class ContextOutro(Context):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.image = font.render('GOODBYE', True, 'orange')
        self.rect = self.image.get_rect(center=(DisplaySettings.width // 2, DisplaySettings.height // 2))

    def update(self, dt):
        pygame.display.set_caption(f'Context: Outro (press any key) | {int(round(Runtime.clock.get_fps()))} fps')
        self.handle_events()
